...
void saveAllData() {
	if (trQuestVarGet("symphonyMode") == 0) {
...
inactive
{
	...
	*/
	if ((trGetScenarioUserData(VERSION_NUMBER) < CURRENT_VERSION) && (trGetScenarioUserData(VERSION_NUMBER) > 0)) {
		uiMessageBox("New endgame mode added: Symphony of War. (Beat the game to unlock)");
