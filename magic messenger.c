...
void magicMessengerAlways(int eventID = -1) {
	...
	if (trQuestVarGet("p"+p+"rainStatus") == ABILITY_ON) {
		...
		for(y=yGetDatabaseCount("p"+p+"characters"); >0) {
			...
			dist = xsPow(trQuestVarGet("tempAbilityRadius") * trQuestVarGet("p"+p+"spellRange"), 2);
			for(x=3; >0) {
				...
				for(i=yGetDatabaseCount("enemies"); >0) {
					...
					if (id == -1 || trUnitAlive() == false) {
						...
						damageEnemy(p, trQuestVarGet("tempAbilityDamage") * trQuestVarGet("p"+p+"spellDamage"));
						if (trUnitAlive()) {
...
void chooseMagicMessenger(int eventID = -1) {
	...
	trQuestVarSet("p"+p+"rainCooldown", trQuestVarGet("tempAbilityCooldown"));
...
highFrequency
{
	...
	trQuestVarSet("tempAbilityCooldown", 15);
	trQuestVarSet("tempAbilityDamage", 50);
	trQuestVarSet("tempAbilityRadius", 5);
