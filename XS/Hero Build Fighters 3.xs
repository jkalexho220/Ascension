...
code("const int THUNDERCALLER = 3;");
...
code("void setupPlayerProto(string proto = \"\", float health = 0, float attack = 0, float range = 12, float speed = 4) {");
...
code("void setupClass(string proto = \"\", int class = 0, int firstDelay = 0, int nextDelay = 0) {");
...
code("setupPlayerProto(\"Militia\", 100, 10, 0, 4.8);");
code("setupClass(\"Hero Greek Theseus\", MOONBLADE, 460, 1000);");
code("setupPlayerProto(\"Hero Greek Theseus\", 1000, 50, 0, 4.3);");
code("setupClass(\"Hero Greek Hippolyta\", SUNBOW, 1350, 1750);");
code("setupPlayerProto(\"Hero Greek Hippolyta\", 1000, 50, 16, 4.3);");
code("setupClass(\"Royal Guard Hero\", THUNDERCALLER, 600, 1000);");
code("setupPlayerProto(\"Royal Guard Hero\", 1200, 30, 0, 4.6);");
...
code("trSoundPlayFN(\"default\",\"1\",-1,\"Building stage...\",\"\");");
...
code("trQuestVarSet(\"enemyDensity\", 0.04 + 0.01 * ENEMY_PLAYER);");
...
code("trStringQuestVarSet(\"enemyProto4\", \"Scarab\");");
...
code("} else if (trQuestVarGet(\"chest\") == 1) {");
...
code("yRemoveUpdateVar(\"playerCharacters\", \"firstDelay\");");
code("yRemoveUpdateVar(\"playerCharacters\", \"nextDelay\");");
code("}");
...
code("float damageEnemy(int p = 0, float dmg = 0, bool spell = false) {");
...
code("void OnHitEffects(int p = 0, int attacker = 0, int target = 0) {");
...
code("yAddUpdateVar(\"playerCharacters\", \"firstDelay\", trQuestVarGet(\"class\"+class+\"firstDelay\"));");
code("yAddUpdateVar(\"playerCharacters\", \"nextDelay\", trQuestVarGet(\"class\"+class+\"nextDelay\"));");
...
code("ySetVar(\"playerCharacters\", \"attackNext\", trTimeMS() + yGetVar(\"playerCharacters\", \"firstDelay\"));");
code("}");
code("} else {");
code("if ((action == 12) || (action == 6)) {");
code("if (trTimeMS() > yGetVar(\"playerCharacters\", \"attackNext\")) {");
code("ySetVar(\"playerCharacters\", \"attackNext\",");
code("yGetVar(\"playerCharacters\", \"attackNext\") + yGetVar(\"playerCharacters\", \"nextDelay\"));");
code("xsSetContextPlayer(p);");
code("target = trGetUnitScenarioNameNumber(kbUnitGetTargetUnitID(id));");
code("OnHitEffects(p, 1*trQuestVarGet(\"playerCharacters\"), target);");
code("if (action == 6) {");
