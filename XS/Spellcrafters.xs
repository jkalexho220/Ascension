...
code("void arrowHitEnemy(int p = 0, int id = 0) {");
...
code("}");
code("if ((yGetVar(\"arrowsActive\", \"element\") == DARK) && kbUnitGetCurrentHitpoints(id) > 0) {");
...
code("if (zDistanceToVectorSquared(\"enemies\", \"hitbox\") < 4) {");
code("arrowHitEnemy(p, kbGetBlockID(\"\"+1*trQuestVarGet(\"enemies\"), true));");
code("}");
code("}");
code("}");
code("} else {");
