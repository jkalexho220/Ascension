...
code("trChangeTerrainHeight(a, 1*trQuestVarGet(\"z\"), a+1, 1*trQuestVarGet(\"z\")+1, 10, false);");
code("}");
code("}");
code("for(b=z0; < z1) {");
code("trQuestVarSetFromRand(\"deploy\", 0, 1, false);");
...
code("trChangeTerrainHeight(1*trQuestVarGet(\"x\"), b, 1*trQuestVarGet(\"x\")+1, b+1, 10, false);");
code("}");
code("}");
code("}");
code("}");
...
code("void updateGold() {");
code("trSetCounterDisplay(\"Gold: \" + 1*trQuestVarGet(\"myGold\"));");
code("}");
...
code("trQuestVarSet(\"myGold\", yGetVar(\"enemies\", \"bounty\") + trQuestVarGet(\"myGold\"));");
code("updateGold();");
code("}");
...
code("trSetSelectedScale(3, 3, range * 1.3);");

code("yAddToDatabase(\"playerLasers\", \"next\");");
code("yAddUpdateVar(\"playerLasers\", \"range\", range * 1.3);");
...
code("trQuestVarSet(\"myGold\", trGetScenarioUserData(9));");
code("xsDisableSelf();");
code("}");
code("}");
...
code("if (1*yGetVar(\"arrowsActive\", \"special\") == FIRE) {");
code("dist = 36;");
code("} else {");
code("dist = 9;");
code("}");
