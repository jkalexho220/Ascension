...
code("modularCounterInit(\"spyFind\", 16);");
code("modularCounterInit(\"spyFound\", 16);");
...
code("ySetVar(db, \"stunSFX\", 0 - spyEffect(1*trQuestVarGet(db), \"Shockwave stun effect\"));");
code("}");
...
code("int p = 0;");
code("float dist = 0;");
code("for (i = 1; < ENEMY_PLAYER / 4) {");
code("trQuestVarSet(\"searchplayer\", 1 + trQuestVarGet(\"searchplayer\"));");
code("if (trQuestVarGet(\"searchplayer\") >= ENEMY_PLAYER) {");
code("trQuestVarSet(\"searchplayer\", 1);");
code("}");
code("p = trQuestVarGet(\"searchplayer\");");
code("id = kbGetBlockID(\"\"+1*trQuestVarGet(\"p\"+p+\"unit\"), true);");
code("if (trQuestVarGet(\"p\"+p+\"attacking\") == 0) {");
code("if (kbUnitGetAnimationActionType(id) == 12) {");
code("trQuestVarSet(\"p\"+p+\"attacking\", 1);");
code("trQuestVarSet(\"p\"+p+\"attackNext\", trTimeMS() + 500);");
code("}");
code("} else {");
code("if (kbUnitGetAnimationActionType(id) == 12) {");
code("if (trTimeMS() > trQuestVarGet(\"p\"+p+\"attackNext\")) {");
code("trQuestVarSet(\"p\"+p+\"attackNext\", trTimeMS() + 1000 / trQuestVarGet(\"p\"+p+\"attackSpeed\"));");
code("xsSetContextPlayer(p);");
code("target = trGetUnitScenarioNameNumber(kbUnitGetTargetUnitID(id));");
code("trVectorSetUnitPos(\"start\", \"p\"+p+\"unit\");");
code("trVectorQuestVarSet(\"end\", kbGetBlockPosition(\"\"+target, true));");

code("trQuestVarSet(\"p\"+p+\"specialAttack\", trQuestVarGet(\"p\"+p+\"specialAttack\") - 1);");
code("if (trQuestVarGet(\"p\"+p+\"specialAttack\") == 0) {");
code("trQuestVarSet(\"p\"+p+\"specialAttack\", trQuestVarGet(\"p\"+p+\"specialAttackCooldown\"));");
code("shootArrow(p, 1*trQuestVarGet(\"p\"+p+\"element1\"), \"start\", \"end\",");
code("trQuestVarGet(\"p\"+p+\"attack\"), 1*trQuestVarGet(\"p\"+p+\"element2\"));");
code("switch(1*trQuestVarGet(\"p\"+p+\"element2\"))");
code("{");
code("case LIGHT:");
code("{");
code("}");
...
code("}");
code("} else {");
code("shootArrow(p, 1*trQuestVarGet(\"p\"+p+\"element1\"), \"start\", \"end\", trQuestVarGet(\"p\"+p+\"attack\"));");
code("}");
code("}");
code("} else {");
code("trQuestVarSet(\"p\"+p+\"attacking\", 0);");
code("}");
code("}");
code("}");
...
code("} else if (trCountUnitsInArea(\"\"+1*trQuestVarGet(\"arrowsActive\"),ENEMY_PLAYER,\"Unit\",2) > 0) {");
...
code("trDamageUnit(yGetVar(\"arrowsActive\", \"damage\"));");
