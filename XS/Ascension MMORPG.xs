...
code("int ySetPointer(string db = \"\", int index = 0) {");
code("int safe = trQuestVarGet(\"xdata\"+db+\"pointer\");");
code("trQuestVarSet(\"xdata\"+db+\"pointer\", index);");
...
code("setupClass(\"Archer Atlantean Hero\", STORMCUTTER, 400, 1000, LIFESTONE);");
code("setupClass(\"Pharaoh\", ALCHEMIST, 550, 1200, LIFESTONE);");
...
code("}");
code("savedata = xsMax(0, trGetScenarioUserData(8));");
...
code("const int NPC_EXPLAIN_SINGLEPLAYER = 1;");
code("const int NPC_ZENO_NEXT_QUESTION = 2;");
code("const int NPC_ZENO_QUIZ_END = 3;");
...
code("case 2:");
code("{");
code("msg = \"Bonus: 0.1 x Attack for every meter traveled.\";");
code("}");
code("case 3:");
code("{");
...
code("{");
code("icon = \"icons\god power shifting sand icon 64\";");
code("msg = \"Cooldown: 8 | Radius: 4 | Range: 12\";");
code("}");
code("case 6:");
code("{");
...
code("{");
code("icon = \"icons\god power call to arms icon\";");
code("msg = \"Cost: 50 | Decay: 1 + 1 per second\";");
code("next = 0;");
code("}");
code("}");
code("}");
...
code("ySetPointer(db, 1*yGetVar(\"launchedUnits\", \"index\"));");
code("stunUnit(db, 1.5);");
...
code("}");
code("trChatSend(p, \"newly added unit is \" + trQuestVarGet(\"p\"+p+\"unit\"));");
code("}");
...
code("trQuestVarGet(\"class\"+SUNBOW+\"nextDelay\") * 4.0 / (3.0 + trQuestVarGet(\"p\"+p+\"projectiles\")));");
code("}");
...
code("removePlayerUnit();");
code("} else {");
code("amt = zDistanceToVectorSquared(\"playerCharacters\", \"p\"+p+\"wellPos\");");
code("if (amt < dist) {");
code("dist = amt;");
...
code("trChatSend(0, \"Removing fire knight from database: \" + yGetPointer(\"p\"+p+\"characters\"));");
code("removePlayerSpecific(p);");
code("yRemoveUpdateVar(\"p\"+p+\"characters\", \"phoenix\");");
code("trChatSend(0, \"database size is now \" + yGetDatabaseCount(\"p\"+p+\"characters\"));");
code("}");
...
code("hit = ySetPointer(\"p\"+p+\"characters\", 1*yGetVar(\"p\"+p+\"fireCharges\", \"index\"));");
code("trUnitSelectClear();");
code("trUnitSelectByQV(\"p\"+p+\"characters\");");
...
code("trChatSend(0, \"database count is \" + yGetDatabaseCount(\"p\"+p+\"characters\"));");
...
code("ySetPointer(\"p\"+p+\"characters\", hit);");
code("} else {");
...
code("id = yDatabaseNext(\"p\"+p+\"characters\", true);");
code("if (id == -1 || trUnitAlive() == false) {");
code("trChatSend(p, \"id is \" + id + \" and name is \" + trQuestVarGet(\"p\"+p+\"characters\"));");
...
code("setupProtounitBounty(\"Fenris Wolf\", 5, 0.03);");
code("setupProtounitBounty(\"Valkyrie\", 7, 0.05);");
code("setupProtounitBounty(\"Ballista\", 6, 0.03);");
code("setupProtounitBounty(\"Frost Giant\", 12, 0.1);");
...
code("equipRelicsAgain(p);");
code("trChatSend(p, \"database size is now \" + yGetDatabaseCount(\"p\"+p+\"characters\"));");
code("}");
...
code("trCounterAddTime(\"bosshealth\",-1,-9999,\"<color={Playercolor(2)}>Wraithwood: 100\", -1);");
...
code("trCounterAddTime(\"bosshealth\",-1,-9999,\"<color={Playercolor(2)}>Wraithwood: \"+1*trQuestVarGet(\"bossHealth\"), -1);");
code("trUnitHighlight(0.2,false);");
code("} else {");
code("for(x=yGetDatabaseCount(\"playerUnits\"); >0) {");
code("id = yDatabaseNext(\"playerUnits\", true);");
